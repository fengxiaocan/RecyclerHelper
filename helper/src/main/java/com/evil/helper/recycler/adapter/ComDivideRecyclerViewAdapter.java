...
public abstract class ComDivideRecyclerViewAdapter<T extends IRecycleData,V extends DivideRecyclerViewHolder<T>> extends RecyclerView.Adapter<BaseRecyclerHolder> implements IExtendAdapter<T> {
	...
	public int getItemCount() {
		...
		int divideCount = getDivideCount();
		return divideCount+ getHeaderCount() + getFooterCount();
	}
	...
	public boolean isFooterOfPosition(int position) {
		return hasFooter() && position >= getDivideCount() + getHeaderCount();
	}
		...
		if (isEmpty() && isHasEmptyView()) {
			...
			if (mOnFooterClickListener != null) {
				holder.itemView.setOnClickListener(new TOnClickListener<Integer>(
						position - (getDivideCount() + getHeaderCount()))
				{
					@Override
					public void onClick(View v) {
			...
			if (holder instanceof DivideRecyclerViewHolder) {
				int realPosition = index * divideNumber();
	...
	public int getItemViewType(int position) {
		...
		if (isHeaderOfPosition(position)) {
			...
			return mFooters.get(position - (getDivideCount() + getHeaderCount())).getViewType();
		}
		else {
