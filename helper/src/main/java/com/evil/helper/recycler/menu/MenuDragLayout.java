...
public class MenuDragLayout extends FrameLayout {
        ...
        public void onViewPositionChanged(View changedView,int left,int top,int dx,int dy) {
            ...
            if (changedView == contentView) {
                dragView.layout(
                        dragView.getLeft() + dx,
                        dragView.getTop() + dy,
                        dragView.getRight() + dx,
                        dragView.getBottom() + dy
                );
            ...
            if (contentView.getLeft() != 0) {
                SwipeMenuManager.getInstance().setMenuDragLayout(MenuDragLayout.this);
            } else {
    ...
    public MenuDragLayout(Context context,AttributeSet attrs) {
        this(context,attrs,0);
    }

    public MenuDragLayout(Context context,AttributeSet attrs,int defStyleAttr) {
        super(context,attrs,defStyleAttr);
        init();
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public MenuDragLayout(Context context,AttributeSet attrs,int defStyleAttr,int defStyleRes) {
    ...
    protected void onLayout(boolean changed,int left,int top,int right,int bottom) {
        ...
        if (dragView != null) {
            dragView.layout(contentWidth,0,contentWidth + deleteWidth,deleteWidth);
        }
    }
